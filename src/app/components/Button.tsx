...
export default function Button({ name, source, color, href, hidden, projectsButtons, heroButtons, aboutButtons, hov }: ButtonProps) {
    return (
        <a href={href} target="_blank" rel="noopener noreferrer" className={`flex items-center ${projectsButtons && "w-[181px] h-[35px]"}  ${heroButtons && "md:w-36 h-8"} ${aboutButtons && "w-[150px] h-[35px]"} rounded ${color} text-white py-5 ${hov && "hover:scale-105"} transition-transform duration-300 `}>
